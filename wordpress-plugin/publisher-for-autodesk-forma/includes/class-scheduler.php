...
class Scheduler {
	...
	public static function intervals() {
		return array(
			'none'       => __( 'Disabled', 'publisher-for-autodesk-forma' ),
			'hourly'     => __( 'Hourly', 'publisher-for-autodesk-forma' ),
			'twicedaily' => __( 'Twice daily', 'publisher-for-autodesk-forma' ),
			'daily'      => __( 'Daily', 'publisher-for-autodesk-forma' ),
		);
	}
	...
	public function run_sync_refresh() {
		...
		if ( null === $connection ) {
			$this->audit_log->log(
				array(
					...
					'result'    => 'error',
					'message'   => __( 'The configured sync connection is missing or disabled.', 'publisher-for-autodesk-forma' ),
				)
			);
		...
		$this->audit_log->log(
			array(
				...
				'message'    => sprintf( __( 'Backend responded with HTTP %d.', 'publisher-for-autodesk-forma' ), $code ),
				'connection' => $key_id,
			)
		);
	}
