	...
	public function handle( array $payload, $connection_id ) {
		...
		if ( '' === $source_id ) {
			return new \WP_Error(
				...
				__( 'The payload does not contain a project source id.', 'publisher-for-autodesk-forma' ),
				array( 'status' => 400 )
			);
		}
		...
		switch ( $operation ) {
			...
			default:
				$result = new \WP_Error(
					...
					__( 'The requested operation is not supported.', 'publisher-for-autodesk-forma' ),
					array( 'status' => 400 )
				);
		}
	...
	private function upsert_project( array $project, $connection_id, $mode, $force = false ) {
		...
		if ( $existing && get_post_meta( $existing, '_forma_payload_hash', true ) === $hash ) {
			return array(
				...
				'source_id'    => $source_id,
				'payload_hash' => $hash,
				'message'      => __( 'Payload matches the stored version; nothing to update.', 'publisher-for-autodesk-forma' ),
			);
		}
		...
		if ( ! $force && $existing && Review::has_local_edits( $existing ) ) {
			...
			if ( 'skip' === $policy ) {
				return array(
					...
					'source_id'    => $source_id,
					'payload_hash' => $hash,
					'message'      => __( 'The project was edited in WordPress, so the update was discarded.', 'publisher-for-autodesk-forma' ),
				);
			}
			...
			if ( 'hold' === $policy ) {
				...
				return array(
					...
					'source_id'    => $source_id,
					'payload_hash' => $hash,
					'message'      => __( 'The project was edited in WordPress, so the update is waiting for review.', 'publisher-for-autodesk-forma' ),
				);
			}
		}
	...
	public function apply_held_update( $post_id ) {
		...
		if ( null === $held ) {
			return new \WP_Error(
				...
				__( 'There is no update waiting for review on that project.', 'publisher-for-autodesk-forma' ),
				array( 'status' => 404 )
			);
		}
	...
	private function import_featured_image( $post_id, array $image ) {
		if ( ! $this->settings->get( 'allow_media_import', false ) ) {
			return new \WP_Error(
				...
				__( 'Remote media import is disabled; the featured image was skipped.', 'publisher-for-autodesk-forma' )
			);
		}
		...
		if ( '' === $url ) {
			return new \WP_Error( 'forma_publisher_media_invalid_url', __( 'The featured image URL is not valid.', 'publisher-for-autodesk-forma' ) );
		}
		...
		if ( ! is_string( $host ) || ! in_array( strtolower( $host ), $allowed, true ) ) {
			return new \WP_Error(
				...
				__( 'The featured image host is not on the allowed list; the image was skipped.', 'publisher-for-autodesk-forma' )
			);
		}
	...
	private function trash( $source_id ) {
		...
		if ( ! wp_trash_post( $post_id ) ) {
			return new \WP_Error(
				...
				__( 'The project could not be moved to the trash.', 'publisher-for-autodesk-forma' ),
				array( 'status' => 500 )
			);
		}
