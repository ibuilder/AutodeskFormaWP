...
/*
 * The declaration is guarded so that including this file more than once in a
 * single request cannot raise a fatal error. A fatal here would abort the
 * uninstall part way through and leave a site with half its plugin data gone.
 */
if ( ! function_exists( 'forma_publisher_uninstall_site' ) ) {
	/**
	 * Returns every capability the plugin grants.
	 *
	 * Duplicated from the Capabilities class on purpose: uninstall.php runs
	 * without the plugin bootstrapped, so no autoloader is available.
	 *
	 * @since 1.0.0
	 *
	 * @return string[] Capability names.
	 */
	function forma_publisher_uninstall_capabilities() {
		$capabilities = array( 'forma_manage_settings', 'forma_view_logs' );
	...
	function forma_publisher_uninstall_site() {
		...
		$capabilities = forma_publisher_uninstall_capabilities();
		$roles        = wp_roles();
