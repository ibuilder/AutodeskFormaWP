...
class Dashboard {
	...
	public function register_menu() {
		add_submenu_page(
			Admin::MENU_SLUG,
			__( 'Forma Publisher Overview', 'forma-publisher' ),
			__( 'Overview', 'forma-publisher' ),
			'edit_forma_projects',
		...
		$label = __( 'Review', 'forma-publisher' );
		...
		add_submenu_page(
			Admin::MENU_SLUG,
			__( 'Editorial Review', 'forma-publisher' ),
			$label,
			'edit_forma_projects',
	...
	public function render_overview() {
		if ( ! current_user_can( 'edit_forma_projects' ) ) {
			wp_die( esc_html__( 'You do not have permission to view Forma Publisher.', 'forma-publisher' ) );
		}
		...
		<div class="wrap forma-publisher-admin">
			<h1><?php esc_html_e( 'Forma Publisher Overview', 'forma-publisher' ); ?></h1>

			<?php if ( empty( $enabled ) ) : ?>
				<div class="notice notice-warning">
					<p>
						<?php esc_html_e( 'No enabled connection exists yet, so no backend can publish to this site.', 'forma-publisher' ); ?>
						<a href="<?php echo esc_url( admin_url( 'admin.php?page=forma-publisher-connections' ) ); ?>">
							<?php esc_html_e( 'Create one', 'forma-publisher' ); ?>
						</a>
					</p>
				</div>
			<?php endif; ?>
				...
				<div class="notice notice-info">
					<p>
						...
						printf(
							...
							esc_html( _n( '%s project needs editorial attention.', '%s projects need editorial attention.', $attention, 'forma-publisher' ) ),
							esc_html( number_format_i18n( $attention ) )
						);
						?>
						<a href="<?php echo esc_url( admin_url( 'admin.php?page=forma-publisher-review' ) ); ?>">
							<?php esc_html_e( 'Review now', 'forma-publisher' ); ?>
						</a>
					</p>
				</div>
			<?php endif; ?>
			...
			<div class="forma-publisher-stats">
				...
				$tiles = array(
					array(
						'label' => __( 'Published', 'forma-publisher' ),
						'value' => isset( $counts->publish ) ? (int) $counts->publish : 0,
					),
					array(
						'label' => __( 'Awaiting review', 'forma-publisher' ),
						'value' => $attention,
					),
					array(
						'label' => __( 'Drafts', 'forma-publisher' ),
						'value' => isset( $counts->draft ) ? (int) $counts->draft : 0,
					),
					array(
						'label' => __( 'Enabled connections', 'forma-publisher' ),
						'value' => count( $enabled ),
					),
					array(
						'label' => __( 'Recent failures', 'forma-publisher' ),
						'value' => $failures,
					),
				);
			...
			<?php if ( '' !== $last_error ) : ?>
				<div class="notice notice-error">
					<p>
						<strong><?php esc_html_e( 'Most recent failure:', 'forma-publisher' ); ?></strong>
			...
			<div class="forma-publisher-panel">
				<h2><?php esc_html_e( 'Pipeline', 'forma-publisher' ); ?></h2>
				<table class="widefat striped">
					<tbody>
						<tr>
							<th scope="row"><?php esc_html_e( 'Ingest endpoint', 'forma-publisher' ); ?></th>
							<td><code><?php echo esc_html( rest_url( 'forma-publisher/v1/ingest' ) ); ?></code></td>
						</tr>
						<tr>
							<th scope="row"><?php esc_html_e( 'Last accepted publish', 'forma-publisher' ); ?></th>
							<td>
								<?php
								if ( $last_used > 0 ) {
									echo esc_html( wp_date( $date_format, $last_used ) );
								} else {
									esc_html_e( 'No publish has been accepted yet.', 'forma-publisher' );
								}
								?>
							</td>
						</tr>
						<tr>
							<th scope="row"><?php esc_html_e( 'Scheduled refresh', 'forma-publisher' ); ?></th>
							<td>
								<?php
								if ( 'none' === $sync_interval ) {
									...
									printf(
										...
										esc_html__( '%1$s, next run %2$s', 'forma-publisher' ),
										esc_html( $sync_interval ),
										esc_html( wp_date( $date_format, $next_sync ) )
									);
								} else {
									esc_html_e( 'Enabled but not scheduled. Visit any admin page to reschedule.', 'forma-publisher' );
								}
								?>
							</td>
						</tr>
						<tr>
							<th scope="row"><?php esc_html_e( 'Log cleanup', 'forma-publisher' ); ?></th>
							<td>
								<?php
								if ( $next_purge ) {
									echo esc_html( wp_date( $date_format, $next_purge ) );
								} else {
									esc_html_e( 'Not scheduled', 'forma-publisher' );
								}
								?>
							</td>
						</tr>
						<tr>
							<th scope="row"><?php esc_html_e( 'Local edit policy', 'forma-publisher' ); ?></th>
							<td>
								<?php
						...
						<tr>
							<th scope="row"><?php esc_html_e( 'WP-Cron', 'forma-publisher' ); ?></th>
							<td>
								<?php
								if ( defined( 'DISABLE_WP_CRON' ) && DISABLE_WP_CRON ) {
									esc_html_e( 'Disabled in configuration. Ensure a system cron calls wp-cron.php.', 'forma-publisher' );
								} else {
									esc_html_e( 'Enabled. It only fires on site traffic, so a quiet site may refresh late.', 'forma-publisher' );
								}
								?>
							</td>
						</tr>
			...
			<div class="forma-publisher-panel">
				<h2><?php esc_html_e( 'Recent activity', 'forma-publisher' ); ?></h2>
				<?php if ( empty( $log['items'] ) ) : ?>
					<p><?php esc_html_e( 'No publish activity has been recorded yet.', 'forma-publisher' ); ?></p>
				<?php else : ?>
					<table class="widefat striped">
						<thead>
							<tr>
					...
					<p>
						<a href="<?php echo esc_url( admin_url( 'admin.php?page=forma-publisher-logs' ) ); ?>">
							<?php esc_html_e( 'View the full publish log', 'forma-publisher' ); ?>
						</a>
					</p>
				<?php endif; ?>
			</div>
	...
	public function render_review() {
		if ( ! current_user_can( 'edit_forma_projects' ) ) {
			wp_die( esc_html__( 'You do not have permission to review Forma projects.', 'forma-publisher' ) );
		}
		...
		?>
		<div class="wrap forma-publisher-admin">
			<h1><?php esc_html_e( 'Editorial Review', 'forma-publisher' ); ?></h1>

			<?php $this->render_notice(); ?>

			<div class="forma-publisher-panel">
				<h2><?php esc_html_e( 'Updates held because the project was edited here', 'forma-publisher' ); ?></h2>
				<p class="description">
					<?php esc_html_e( 'These projects changed in WordPress after the last synchronization, so the incoming update was not applied automatically.', 'forma-publisher' ); ?>
				</p>

				<?php if ( empty( $held ) ) : ?>
					<p><?php esc_html_e( 'Nothing is waiting. Every project matches its published version.', 'forma-publisher' ); ?></p>
				<?php else : ?>
					<table class="widefat striped">
						<thead>
							<tr>
								<th scope="col"><?php esc_html_e( 'Project', 'forma-publisher' ); ?></th>
								<th scope="col"><?php esc_html_e( 'Incoming title', 'forma-publisher' ); ?></th>
								<th scope="col"><?php esc_html_e( 'Held since', 'forma-publisher' ); ?></th>
								...
								<tr>
									...
									<td>
										<a class="button button-primary" href="<?php echo esc_url( $this->action_url( $post->ID, 'apply' ) ); ?>">
											<?php esc_html_e( 'Apply update', 'forma-publisher' ); ?>
										</a>
										<a class="button" href="<?php echo esc_url( $this->action_url( $post->ID, 'discard' ) ); ?>">
											<?php esc_html_e( 'Keep local edits', 'forma-publisher' ); ?>
										</a>
									</td>
								</tr>
							<?php endforeach; ?>
			...
			<div class="forma-publisher-panel">
				<h2><?php esc_html_e( 'Projects awaiting approval', 'forma-publisher' ); ?></h2>

				<?php if ( empty( $pending ) ) : ?>
					<p><?php esc_html_e( 'No projects are pending review.', 'forma-publisher' ); ?></p>
				<?php else : ?>
					<table class="widefat striped">
						<thead>
							<tr>
								<th scope="col"><?php esc_html_e( 'Project', 'forma-publisher' ); ?></th>
								<th scope="col"><?php esc_html_e( 'Received', 'forma-publisher' ); ?></th>
								<th scope="col"><?php esc_html_e( 'Actions', 'forma-publisher' ); ?></th>
							</tr>
						</thead>
						<tbody>
									...
									<td>
										<?php if ( current_user_can( 'publish_forma_projects' ) ) : ?>
											<a class="button button-primary" href="<?php echo esc_url( $this->action_url( $post->ID, 'approve' ) ); ?>">
												<?php esc_html_e( 'Approve and publish', 'forma-publisher' ); ?>
											</a>
										<?php endif; ?>
										<a class="button" href="<?php echo esc_url( (string) get_permalink( $post->ID ) ); ?>">
											<?php esc_html_e( 'Preview', 'forma-publisher' ); ?>
										</a>
									</td>
								</tr>
							<?php endforeach; ?>
	...
	public function handle_review_action() {
		...
		if ( ! $post_id || ! current_user_can( 'edit_forma_project', $post_id ) ) {
			wp_die( esc_html__( 'You do not have permission to review that project.', 'forma-publisher' ) );
		}
		...
		switch ( $task ) {
			...
			case 'approve':
				if ( ! current_user_can( 'publish_forma_projects' ) ) {
					wp_die( esc_html__( 'You do not have permission to publish Forma projects.', 'forma-publisher' ) );
				}
	...
	private function render_notice() {
		...
		$messages = array(
			'review_applied'   => array( 'success', __( 'The held update was applied.', 'forma-publisher' ) ),
			'review_discarded' => array( 'success', __( 'The held update was discarded and the local version kept.', 'forma-publisher' ) ),
			'review_approved'  => array( 'success', __( 'The project was published.', 'forma-publisher' ) ),
			'review_failed'    => array( 'error', __( 'That review action could not be completed.', 'forma-publisher' ) ),
		);
