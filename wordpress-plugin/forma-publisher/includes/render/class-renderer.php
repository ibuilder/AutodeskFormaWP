...
class Renderer {
	...
	public static function project( array $atts ) {
		...
		if ( ! $post instanceof \WP_Post ) {
			return self::notice( __( 'That Forma project could not be found.', 'forma-publisher' ) );
		}
	...
	public static function asset_kind_label( $kind ) {
		$labels = array(
			'image'    => __( 'Image', 'forma-publisher' ),
			'document' => __( 'Document', 'forma-publisher' ),
			'model'    => __( 'Model', 'forma-publisher' ),
			'dataset'  => __( 'Dataset', 'forma-publisher' ),
			'link'     => __( 'Link', 'forma-publisher' ),
		);

		$kind = (string) $kind;

		return isset( $labels[ $kind ] ) ? $labels[ $kind ] : __( 'File', 'forma-publisher' );
	}
