...
<article class="forma-publisher forma-publisher-project" id="forma-project-<?php echo esc_attr( (string) $forma_publisher_project->ID ); ?>">
	...
	<?php if ( $forma_publisher_show_thumbnail && has_post_thumbnail( $forma_publisher_project ) ) : ?>
		<figure class="forma-publisher-project__media">
			<?php echo get_the_post_thumbnail( $forma_publisher_project, 'large', array( 'loading' => 'lazy' ) ); ?>
		</figure>
	<?php endif; ?>
	...
	if ( ! empty( $forma_publisher_metrics ) ) {
		$forma_publisher_metrics_markup = Forma_Publisher\Templates::render(
			'metrics.php',
			array(
		...
		// Every value is escaped inside metrics.php before it reaches this point.
		echo $forma_publisher_metrics_markup; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}

	if ( ! empty( $forma_publisher_assets ) ) {
		$forma_publisher_assets_markup = Forma_Publisher\Templates::render(
			'assets.php',
			array(
				'project' => $forma_publisher_project,
		...
		// Every value is escaped inside assets.php before it reaches this point.
		echo $forma_publisher_assets_markup; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	?>
		...
		<?php if ( '' !== $forma_publisher_source_url ) : ?>
			<p class="forma-publisher-project__source">
				<a href="<?php echo esc_url( $forma_publisher_source_url ); ?>" rel="nofollow noopener external" target="_blank">
					<?php esc_html_e( 'View in Autodesk Forma', 'forma-publisher' ); ?>
				</a>
			</p>
		<?php endif; ?>
			...
			<?php if ( $forma_publisher_synced_ts ) : ?>
				<p class="forma-publisher-project__synced">
					...
					printf(
						...
						esc_html__( 'Last synchronized %s', 'forma-publisher' ),
